...
class OcManager {
  ...
  // 成员变量
  bool isEdit = false; // 是否为编辑页面
  var configs = <dynamic, dynamic>{}; // 配置项
  var traceId = ''; // 换脸id
  final results = <GenerationResult>[]; // 生成的结果
  ...
  Future<void> init(Map<dynamic, dynamic>? map) async {
    if (map != null) {
      _initWithDraft(map);
    } else {
  ...
  Future<void> _initWithDraft(Map<dynamic, dynamic> map) async {
    final roleInfo =
        map[Security.security_info]?[Security.security_roleInfo] ??
        map[EncHelper.cr_cusri];
    ...
    configs[Security.security_gender] =
        (roleInfo[Security.security_gender] == 1 ||
                roleInfo[Security.security_gender] == 2)
            ? roleInfo[Security.security_gender]
            : 2;
    configs[Security.security_chatBackground] =
        roleInfo[Security.security_chatBackground];
    configs[Security.security_masterUid] =
        roleInfo[Security.security_masterUid] ?? MyAccount.id;
    configs[Security.security_age] = roleInfo[Security.security_age];
    configs[Security.security_avatarUrl] =
        roleInfo[Security.security_avatarUrl];
  ...
  Future<Map?> ocOptions(
    Map<dynamic, dynamic> info,
    int optType,
    List<String> generateImages,
    String changeFaceTraceId,
    int? roleUid,
  ) async {
    final args = {
  ...
  Future<void> save() async {
    final rtn = ocOptions(configs, 3, generateImgs, traceId, null);
  }

  // 发送到服务器更新
  Future<Map?> update() async {
    return await ocOptions(configs, 1, generateImgs, traceId, null);
  }

  Future<Map?> createDraft() async {
    return await OcManager.instance.ocOptions({}, 2, [], '', null);
  }

  // 发送到服务器创建
  Future<Map?> createRole() async {
    return await ocOptions(configs, 0, generateImgs, traceId, null);
  }

  Future<Map?> generateImageBg(
    Map<dynamic, dynamic> info,
    int generateOptType,
  ) async {
    final req = ApiRequest(
  ...
  Future<Map?> getGenResult(String traceId) async {
    final req = ApiRequest(
      EncHelper.cr_ask_gen_rsp,
      params: {EncHelper.cr_cre_id: traceId},
    );
    final rsp = await ApiService.instance.sendRequest(req);
    if (rsp.isSuccess) {
      return rsp.data;
  ...
  Future<Map?> getEditRoleInfo(int uid) async {
    final req = ApiRequest(EncHelper.cr_fed, params: {EncHelper.cr_cccid: uid});
    final rsp = await ApiService.instance.sendRequest(req);
    if (rsp.isSuccess) {
      return rsp.data;
    }
  ...
  Future<bool> createForBgRegeneration() async {
    ...
    if (rtn != null &&
        rtn[Security.security_statusInfo][Security.security_traceId] != null) {
  ...
  Future<void> downloadImage(Map rtn) async {
    ...
    final path =
        '${dir!.path}/temp_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  Future<String> cropped(
    ...
    if (originImage != null) {
      ...
      await File(croppedFilePath).writeAsBytes(
        ext.toLowerCase() == Security.security_png
